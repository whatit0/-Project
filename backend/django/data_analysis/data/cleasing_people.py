...
# 파일 목록 가져오기
file_list = glob.glob("../data/train_people/LOCAL_PEOPLE_DONG_*.csv")
print(file_list)
...
try:
    train_people.to_csv('train_people_processed.csv', index=False)
...
try:
    test_people.to_csv('test_people_processed.csv', index=False)
    print("파일이 성공적으로 저장되었습니다.")
except Exception as e:
    print("파일 저장 중 에러 발생:", e)
