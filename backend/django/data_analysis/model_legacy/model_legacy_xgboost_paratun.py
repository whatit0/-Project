...
rent_search = RandomizedSearchCV(
...
return_search = RandomizedSearchCV(
    estimator=xgb.XGBRegressor(enable_categorical=True),
    param_distributions=param_dist,
    n_iter=25,
    cv=5,
...
# 튜닝 실행
print("Tuning for rent model:")
rent_search.fit(X_train, y1_train)
print("Best parameters for rent model:", rent_search.best_params_)

print("Tuning for return model:")
return_search.fit(X_train, y2_train)
print("Best parameters for return model:", return_search.best_params_)

# 튜닝된 파라미터로 모델 생성 및 학습
xgb_model_rent = xgb.XGBRegressor(**rent_search.best_params_)
xgb_model_return = xgb.XGBRegressor(**return_search.best_params_)
