...
function Map() {
  ...
  const handleSearchClick = (e) => {
    ...
    if (searchTerm.trim() !== "") {
      // 검색 결과 필터링
      const results = markerData.filter(
        (station) =>
          station.stationName.includes(searchTerm) &&
          includedIds.includes(station.stationId)
      );
  ...
  const handlePredictionSubmit = async (e) => {
    ...
    try {
      ...
      const response2 = await axios.get("http://localhost:80/chart", {
        params: {
          stationId: selectedStationId,
        },
      });
      ...
      if (rentElement) {
        rentElement.textContent =
          rent_predictions.length > 0 ? rent_predictions[0] : "0";
      }

      const returnElement = document.querySelector(".return_num");
      if (returnElement) {
        returnElement.textContent =
          return_predictions.length > 0 ? return_predictions[0] : "0";
      }
      ...
      try {
        ...
        for (
          ...
          let response = await axios.get(
            `http://openapi.seoul.go.kr:8088/456852427579656a313035727966656c/json/bikeList/${
          ...
          markerData.forEach((station) => {
            if (includedIds.includes(station.stationId)) {
              ...
              naver.maps.Event.addListener(marker, "click", () => {
                document.getElementById("map_data_info").style.display =
                  "block";
      ...
      <div id="map_info">
        <div id="map_info_left">
          <div className="left_head">
            ...
            <Link to="" className="left_menu left_menu03">
              <div className="flex_center">
          ...
          <div id="map_data_info">
            <div className="map_data_top">
              ...
              <div className="bicycle_info flex">
                ...
                <span className="material-symbols-rounded close_btn">
                  close
                </span>
              </div>
            </div>
                ...
                <form onSubmit={handlePredictionSubmit} id="pred_form">
                  <div className="datepicker date input-group p-0 shadow-sm">
                    ...
                    <div className="input-group-append">
                      <span className="material-symbols-rounded fs20 fw400 dgreen input-group-text px-3">
                        event_available
                      </span>
                    </div>
                  </div>
                  <div className="time">
                    <div className="form-group">
                      <div
                        className="input-group time shadow-sm"
                        id="timepicker"
                      >
                        <input
                          type="text"
                  ...
                  <button type="submit" id="pred_btn">
                    예측하기
                  </button>
                </form>
                <div id="result_cont">
                  ...
                  <div className="result_data flex">
                    <div className="return_data">
                      <p className="flex2">
                        <span className="material-symbols-rounded fs25 fw400 wd22 red">
                          arrow_drop_up
                        </span>
                        <span className="red">예상 반납건수</span>
                      </p>
                      <p>
                        <span className="return_num">0</span>건
                      </p>
                    </div>
                    <div className="rental_data">
                      <p className="flex2">
                        <span className="material-symbols-rounded fs25 fw400 wd22 blue">
                          arrow_drop_down
                        </span>
                        <span className="blue">예상 대여건수</span>
                      </p>
                      <p>
                        <span className="rental_num">0</span>건
                      </p>
                    </div>
                  </div>
        ...
        <div id="map_info_right">
          ...
          <div className="bicycle_icon icon_box" onClick={moveToGangnam}>
            <span className="material-symbols-rounded fs25 fw500">
              directions_bike
            </span>
          </div>
        </div>
